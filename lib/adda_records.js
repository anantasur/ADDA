...
var _addNewTopic = function(newTopic, db, onComplete) {
	var add_topic_query = "insert into topics(name,description,start_time,owner_id) values" +
		" ('" + newTopic.topic_name + "','" + newTopic.topic_desc + "','" + newTopic.start_time + "'," + newTopic.owner_id + ");"
...
var _insertIntoJoinedUsers = function(detail, db, onComplete) {
	var insert_user_query = 'insert into joinedUsers values(' + detail.user_id + ', ' + detail.topic_id + ')';
	db.run(insert_user_query, onComplete);
}

var _deleteFromJoinedUsers = function(detail, db, onComplete) {
	var delete_user_query = 'delete from joinedUsers where user_id=' + detail.user_id +
		' and topic_id=' + detail.topic_id;
	db.run(delete_user_query, onComplete);
}

var _updateEndTimeIntoTopics = function(detail,db,onComplete){
	var insert_endTime_query = "update topics set end_time='" + detail.end_time + "' where id="+detail.topic_id; 
	db.run(insert_endTime_query,onComplete);
}
...
var init = function(location) {
	...
	var records = {
		...
		insertIntoJoinedUsers: operate(_insertIntoJoinedUsers),
		deleteFromJoinedUsers: operate(_deleteFromJoinedUsers),
		updateEndTimeIntoTopics : operate(_updateEndTimeIntoTopics)
	};
